// Package watcher monitors the CraftPlanner data directory for changes to
...
// Watcher monitors a CraftPlanner data directory for manifest file changes
...
func (w *Watcher) handleEvent(ctx context.Context, event fsnotify.Event) {
		...
		if info, err := os.Stat(event.Name); err == nil && info.IsDir() {
			_ = filepath.WalkDir(event.Name, func(path string, d fs.DirEntry, err error) error {
				...
				if d.IsDir() {
					...
					w.schedule(ctx, path)
				} else if strings.HasSuffix(filepath.Base(path), ".meta.json") && strings.Contains(path, "/assets/") {
					w.scheduleAsset(ctx, path)
				}
				return nil
			})
			return
	...
	// Handle asset metadata files (.meta.json in assets/ directories).
	if strings.HasSuffix(base, ".meta.json") && strings.Contains(event.Name, "/assets/") {
		w.scheduleAsset(ctx, event.Name)
		return
	}

	// Only care about configured manifest files; ignore everything else.
	if base != w.manifestFilename {
		return
	}
...
// scheduleAsset arms a debounce timer for an asset metadata file.
func (w *Watcher) scheduleAsset(ctx context.Context, absPath string) {
	w.mu.Lock()
	key := "asset:" + absPath
	...
	w.pending[key] = &pendingEntry{
		timer: time.AfterFunc(debounceDelay, func() {
			...
			w.processAsset(ctx, absPath)
		}),
	}
	w.mu.Unlock()
}

// processAsset handles a changed or deleted asset metadata file.
func (w *Watcher) processAsset(ctx context.Context, absPath string) {
	rel, err := filepath.Rel(w.dataDir, absPath)
	if err != nil {
		log.Printf("watcher: asset rel path for %s: %v", absPath, err)
		return
	}
	...
	data, err := os.ReadFile(absPath)
	if os.IsNotExist(err) {
		// Asset deleted — remove from index.
		if err := w.idx.DeleteAsset(ctx, rel); err != nil {
			log.Printf("watcher: delete asset %s: %v", rel, err)
		} else {
			log.Printf("watcher: removed asset %s from index", rel)
		}
		return
	}
	if err != nil {
		log.Printf("watcher: read asset %s: %v", rel, err)
		return
	}

	// Parse the metadata and upsert into the assets table.
	var meta map[string]json.RawMessage
	if err := json.Unmarshal(data, &meta); err != nil {
		log.Printf("watcher: parse asset %s: %v", rel, err)
		return
	}

	// Derive the primitive directory path from the asset path.
	// e.g. "materials/leather/assets/photo.jpg.meta.json" → "materials/leather"
	parts := strings.Split(rel, "/assets/")
	primDir := parts[0]

	filename := ""
	if raw, ok := meta["filename"]; ok {
		json.Unmarshal(raw, &filename)
	}
	...
	if err := w.idx.UpsertAsset(ctx, a); err != nil {
		log.Printf("watcher: index asset %s: %v", rel, err)
		return
	}
	log.Printf("watcher: indexed asset %s for %s", rel, primDir)
}
