...
// writeFedConfig writes a federation.json under dir/.craftplanner/ and returns
// the dir path. Creates the .craftplanner subdirectory automatically.
func writeFedConfig(t *testing.T, dir string, v any) {
	t.Helper()
	msDir := filepath.Join(dir, ".craftplanner")
...
func TestRoot_ParserConfigFile_DefaultsToMakestackParser(t *testing.T) {
	r := Root{Slug: "primary"}
	if got := r.ParserConfigFile(); got != "craftplanner-parser.json" {
		t.Errorf("ParserConfigFile() = %q, want %q", got, "craftplanner-parser.json")
	}
}
