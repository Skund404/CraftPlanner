// Package federation manages the multi-root configuration for craftplanner-core.
...
type Root struct {
	...
	// Defaults to "craftplanner-parser.json" when absent.
...
// Path. Falls back to "craftplanner-parser.json" when not set.
func (r *Root) ParserConfigFile() string {
	if r.ParserConfig != "" {
		return r.ParserConfig
	}
	return "craftplanner-parser.json"
}
...
const configFile = ".craftplanner/federation.json"

// LoadConfig reads the federation config from .craftplanner/federation.json
