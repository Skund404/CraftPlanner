// Package schema validates the structural correctness of CraftPlanner manifest
...
func Validate(primType string, body map[string]json.RawMessage) []string {
	...
	case "template":
		// Templates must declare which primitive types they apply to.
		if raw, ok := body["applies_to"]; ok {
			errs = append(errs, checkStringArrayField("applies_to", raw)...)
		}
		// Templates must have sections with field definitions.
		if raw, ok := body["sections"]; ok {
			errs = append(errs, checkTemplateSections(raw)...)
		}
	}

	// — custom_fields validation (any primitive type) —————————————————————
	if raw, ok := body["custom_fields"]; ok {
		errs = append(errs, checkCustomFields(raw)...)
	}

	return errs
}

// validUnitTypes is the set of recognised unit type identifiers.
var validUnitTypes = map[string]bool{
	"length": true, "weight": true, "volume": true, "temperature": true,
	"area": true, "density": true, "currency": true, "text": true,
}

// checkCustomFields validates the custom_fields object.
// Expected shape: { "_template": "str", "sections": [ { "name": "str", "fields": [...] } ] }
func checkCustomFields(raw json.RawMessage) []string {
	var obj map[string]json.RawMessage
	if err := json.Unmarshal(raw, &obj); err != nil {
		return []string{"custom_fields: must be an object"}
	}

	var errs []string

	// _template is optional; if present, must be string.
	if tpl, ok := obj["_template"]; ok {
		if !isString(tpl) {
			errs = append(errs, "custom_fields._template: must be a string")
		}
	}

	// sections is required.
	sectionsRaw, hasSections := obj["sections"]
	if !hasSections {
		return append(errs, "custom_fields: missing required field \"sections\"")
	}

	errs = append(errs, checkFieldSections("custom_fields", sectionsRaw)...)
	return errs
}

// checkFieldSections validates an array of section objects, each with
// "name" (string) and "fields" (array of field objects).
func checkFieldSections(prefix string, raw json.RawMessage) []string {
	var sections []json.RawMessage
	if err := json.Unmarshal(raw, &sections); err != nil {
		return []string{prefix + ".sections: must be an array"}
	}

	var errs []string
	for i, secRaw := range sections {
		sp := fmt.Sprintf("%s.sections[%d]", prefix, i)
		var sec map[string]json.RawMessage
		if err := json.Unmarshal(secRaw, &sec); err != nil {
			errs = append(errs, sp+": must be an object")
			continue
		}

		if nameRaw, ok := sec["name"]; !ok || !isString(nameRaw) || stringVal(nameRaw) == "" {
			errs = append(errs, sp+`: missing required field "name"`)
		}

		fieldsRaw, hasFields := sec["fields"]
		if !hasFields {
			errs = append(errs, sp+`: missing required field "fields"`)
			continue
		}

		var fields []json.RawMessage
		if err := json.Unmarshal(fieldsRaw, &fields); err != nil {
			errs = append(errs, sp+`.fields: must be an array`)
			continue
		}

		for j, fRaw := range fields {
			fp := fmt.Sprintf("%s.fields[%d]", sp, j)
			errs = append(errs, checkFieldDef(fp, fRaw)...)
		}
	}
	return errs
}

// checkFieldDef validates a single custom field definition.
// Required: "key" (string), "label" (string), "unit_type" (valid unit type).
// Optional: "value" (any), "unit" (string), "default_unit" (string).
func checkFieldDef(prefix string, raw json.RawMessage) []string {
	var obj map[string]json.RawMessage
	if err := json.Unmarshal(raw, &obj); err != nil {
	...
	if keyRaw, ok := obj["key"]; !ok || !isString(keyRaw) || stringVal(keyRaw) == "" {
		errs = append(errs, prefix+`: missing required field "key"`)
	}
	if labelRaw, ok := obj["label"]; !ok || !isString(labelRaw) || stringVal(labelRaw) == "" {
		errs = append(errs, prefix+`: missing required field "label"`)
	}

	if utRaw, ok := obj["unit_type"]; ok {
		if !isString(utRaw) {
			errs = append(errs, prefix+`: "unit_type" must be a string`)
		} else if ut := stringVal(utRaw); !validUnitTypes[ut] {
			errs = append(errs, fmt.Sprintf(
				`%s: "unit_type" must be one of length, weight, volume, temperature, area, density, currency, text; got %q`,
				prefix, ut))
		}
	}

	if unitRaw, ok := obj["unit"]; ok && unitRaw != nil {
		if !isString(unitRaw) {
			errs = append(errs, prefix+`: "unit" must be a string`)
		}
	}
...
// checkTemplateSections validates the sections field for template primitives.
// Same structure as custom_fields sections but fields have default_unit instead of value.
func checkTemplateSections(raw json.RawMessage) []string {
	return checkFieldSections("sections", raw)
}

// checkStringArrayField validates that raw is a JSON array of strings.
func checkStringArrayField(name string, raw json.RawMessage) []string {
	var elems []json.RawMessage
	if err := json.Unmarshal(raw, &elems); err != nil {
		return []string{name + ": must be an array of strings"}
	}
	var errs []string
	for i, elem := range elems {
		if !isString(elem) {
			errs = append(errs, fmt.Sprintf("%s[%d]: element must be a string", name, i))
		}
	}
	return errs
}
