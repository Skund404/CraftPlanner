// Package index manages the SQLite read index for CraftPlanner primitives.
...
var schemaStmts = []string{
	...
	// Asset metadata table — tracks binary file metadata per primitive.
	...
	`CREATE INDEX IF NOT EXISTS idx_assets_primitive ON assets(primitive_path)`,
}

// Primitive holds all indexed fields for a single CraftPlanner primitive.
...
// Index is a SQLite-backed read index of CraftPlanner primitives.
type Index struct {
	db *sql.DB
}
...
// Asset holds a single row from the assets table.
type Asset struct {
	ID            int64  `json:"id"`
	PrimitivePath string `json:"primitive_path"`
...
// ListAssets returns all assets for a primitive identified by its directory
// path (e.g. "materials/leather" or "materials/leather/manifest.json").
func (idx *Index) ListAssets(ctx context.Context, primPath string) ([]Asset, error) {
	// Normalise: accept both directory path and manifest path.
	primPath = strings.TrimSuffix(primPath, "/manifest.json")
	// Assets are stored with paths like "materials/leather/assets/photo.jpg.meta.json"
	// so we match on primitive_path.
	rows, err := idx.db.QueryContext(ctx,
		`SELECT id, primitive_path, asset_path, filename, mime_type, size_bytes, checksum, metadata, created
		 FROM assets WHERE primitive_path = ? ORDER BY created`,
		primPath)
	if err != nil {
		return nil, fmt.Errorf("list assets for %s: %w", primPath, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var a Asset
		var meta string
		if err := rows.Scan(&a.ID, &a.PrimitivePath, &a.AssetPath, &a.Filename,
			&a.MimeType, &a.SizeBytes, &a.Checksum, &meta, &a.Created); err != nil {
			return nil, err
		}
...
// UpsertAsset inserts or updates an asset metadata record.
func (idx *Index) UpsertAsset(ctx context.Context, a Asset) error {
	_, err := idx.db.ExecContext(ctx, `
		INSERT INTO assets (primitive_path, asset_path, filename, mime_type, size_bytes, checksum, metadata, created)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(asset_path) DO UPDATE SET
			...
			metadata=excluded.metadata, created=excluded.created`,
		a.PrimitivePath, a.AssetPath, a.Filename, a.MimeType,
		a.SizeBytes, a.Checksum, string(a.Metadata), a.Created)
	return err
}

// DeleteAsset removes an asset record by its asset path.
func (idx *Index) DeleteAsset(ctx context.Context, assetPath string) error {
	_, err := idx.db.ExecContext(ctx, `DELETE FROM assets WHERE asset_path = ?`, assetPath)
	return err
}
