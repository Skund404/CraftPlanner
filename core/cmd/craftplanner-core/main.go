// craftplanner-core is the headless data management engine for CraftPlanner.
...
import (
	...
	"github.com/Skund404/CraftPlanner/core/internal/api"
	...
	"github.com/Skund404/CraftPlanner/core/internal/watcher"
)

func main() {
	dataDir     := flag.String("data", "", "Path to the primary CraftPlanner data repository (required)")
	addr        := flag.String("addr", ":8420", "Address to listen on")
	dbPath      := flag.String("db", ":memory:", "SQLite index path (use :memory: for ephemeral)")
	apiKeyFlag  := flag.String("api-key", "", "API key for authentication (overrides CRAFTPLANNER_API_KEY env var)")
	...
	apiKey := *apiKeyFlag
	if apiKey == "" {
		apiKey = os.Getenv("CRAFTPLANNER_API_KEY")
	}
	if apiKey == "" {
	...
	// If .craftplanner/federation.json is absent, a single-root default is returned
	...
	// — Bulk-load: index asset metadata files ————————————————————————————
	var totalAssets int
	for _, root := range fedConfig.Roots {
		pc := parserCfgs[root.Slug]
		assets, err := loadRootAssets(ctx, &root, pc)
		if err != nil {
			log.Printf("warning: load assets for root %q: %v", root.Slug, err)
			continue
		}
		for _, a := range assets {
			if err := idx.UpsertAsset(ctx, a); err != nil {
				log.Printf("warning: index asset %s: %v", a.AssetPath, err)
				continue
			}
			totalAssets++
		}
	...
	if totalAssets > 0 {
		log.Printf("indexed %d asset(s)", totalAssets)
	}
	...
	go func() {
		log.Printf("craftplanner-core listening on %s", *addr)
...
// loadRootAssets walks the data directory looking for .meta.json files in
// assets/ subdirectories and returns index.Asset records for each.
func loadRootAssets(ctx context.Context, root *federation.Root, cfg *parser.Config) ([]index.Asset, error) {
	...
	for dirName := range cfg.Index.Directories {
		...
		err := filepath.WalkDir(dirPath, func(path string, d fs.DirEntry, err error) error {
			...
			// Only process .meta.json files inside assets/ directories.
			...
			// Derive primitive directory from asset path.
			parts := strings.Split(rel, "/assets/")
			primDir := parts[0]

			var meta map[string]json.RawMessage
			if err := json.Unmarshal(data, &meta); err != nil {
				log.Printf("warning: parse asset %s: %v", rel, err)
				return nil
			}

			a := index.Asset{
				PrimitivePath: primDir,
				AssetPath:     rel,
				Metadata:      json.RawMessage(data),
			}
			if raw, ok := meta["filename"]; ok {
				json.Unmarshal(raw, &a.Filename)
			}
			if raw, ok := meta["mime_type"]; ok {
				json.Unmarshal(raw, &a.MimeType)
			}
			if raw, ok := meta["size_bytes"]; ok {
				json.Unmarshal(raw, &a.SizeBytes)
			}
			if raw, ok := meta["checksum_sha256"]; ok {
				json.Unmarshal(raw, &a.Checksum)
			}
			if raw, ok := meta["created"]; ok {
				json.Unmarshal(raw, &a.Created)
			}
		...
		if err != nil {
			return nil, fmt.Errorf("walk assets in %s root %q: %w", dirName, root.Slug, err)
		}
	}
