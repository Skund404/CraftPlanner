"""craftplanner — CraftPlanner Shell CLI.

Entry point for all Shell management commands.

Usage:
    craftplanner start              Start the Shell in production mode
    craftplanner dev                Start in dev mode (verbose logging, debug API)
    craftplanner dev --module PATH  Start in dev mode with a local module loaded
    craftplanner mcp                Run the MCP server over stdio (for Claude Code etc.)
    craftplanner module create NAME Scaffold a new module
    craftplanner module validate PATH  Validate a module manifest
    craftplanner rebuild-frontend   Regenerate module registry and rebuild the frontend

Package management (requires Shell to be running):
    craftplanner install <name>     Install a package from a registry
    craftplanner install <url>      Install from a Git URL
    craftplanner install <path>     Install from a local path
    craftplanner uninstall <name>   Uninstall a package
    craftplanner update <name>      Update to latest version
    craftplanner search <query>     Search across all registries
    craftplanner list               List installed packages
    craftplanner registry add <name> <url>  Add a registry
    craftplanner registry list      List configured registries
    craftplanner registry remove <name>    Remove a registry
    craftplanner registry refresh   Pull latest from all registries
"""
...
@click.group()
@click.version_option(version="0.1.0", prog_name="craftplanner")
def app() -> None:
    """CraftPlanner Shell — maker's modular project management toolkit."""
...
@click.option("--core-url", default="http://localhost:8420", show_default=True,
              help="craftplanner-core URL")
@click.option("--userdb", default="~/.craftplanner/userdb.sqlite", show_default=True,
              help="Path to the UserDB SQLite file")
def start(port: int, core_url: str, userdb: str) -> None:
    """Start the CraftPlanner Shell in production mode."""
    import os
    import uvicorn

    os.environ.setdefault("CRAFTPLANNER_PORT", str(port))
    os.environ.setdefault("CRAFTPLANNER_CORE_URL", core_url)
    os.environ.setdefault("CRAFTPLANNER_USERDB_PATH", userdb)
    os.environ["CRAFTPLANNER_DEV_MODE"] = "false"

    click.echo(f"Starting CraftPlanner Shell on port {port}…")
    uvicorn.run(
...
def dev(port: int, core_url: str, userdb: str, module_path: str | None) -> None:
    """Start the CraftPlanner Shell in development mode.
    ...
    os.environ["CRAFTPLANNER_PORT"] = str(port)
    os.environ["CRAFTPLANNER_CORE_URL"] = core_url
    os.environ["CRAFTPLANNER_USERDB_PATH"] = userdb
    os.environ["CRAFTPLANNER_DEV_MODE"] = "true"

    if module_path:
        _register_dev_module(module_path, userdb)

    click.echo(f"Starting CraftPlanner Shell in dev mode on port {port}…")
    if module_path:
...
# ---------------------------------------------------------------------------
# craftplanner module (subcommand group)
# ---------------------------------------------------------------------------
...
def module_create(name: str, output: str) -> None:
    ...
    The scaffolded module passes 'craftplanner module validate' immediately.
    """
    import re
    from pathlib import Path
    ...
    _write(module_dir / "craftplanner-package.json", _render(_PACKAGE_TMPL, subs))
    ...
    click.echo(f"  3. craftplanner module validate {module_dir}")
    click.echo(f"  4. craftplanner dev --module {module_dir}")
...
def mcp() -> None:
    ...
    Configure via CRAFTPLANNER_SHELL_URL (default: http://localhost:3000).
...
_MANIFEST_TMPL = """\
{
  ...
  "version": "0.1.0",
  "description": "A CraftPlanner module.",
  "author": "",
  "license": "proprietary",
...
from craftplanner_sdk.testing import MockUserDB, create_test_app
...
def _shell_url() -> str:
    import os
    return os.getenv("CRAFTPLANNER_SHELL_URL", "http://localhost:3000")
...
def _api(method: str, path: str, **kwargs) -> dict:
    ...
    try:
        ...
        click.echo(
            ...
            "Is the Shell running? Use 'craftplanner start' or 'craftplanner dev'.",
            err=True,
        )
        raise SystemExit(1)
...
# ---------------------------------------------------------------------------
# craftplanner registry (subcommand group)
# ---------------------------------------------------------------------------
...
def registry_list() -> None:
    ...
    if not items:
        ...
        click.echo("Add one with: craftplanner registry add <name> <git_url>")
        return
...
def registry_refresh() -> None:
    ...
    if not refreshed and not errors:
        click.echo("No registries to refresh. Add one with: craftplanner registry add")
...
def cmd_backup() -> None:
    ...
    The backup is stored in ~/.craftplanner/backups/ with a timestamp in the
    filename. Use 'craftplanner restore' to recover from a backup.
    """
...
def cmd_restore(backup_path: str, yes: bool) -> None:
    ...
    BACKUP_PATH is the full path to the backup file. Use 'craftplanner backup'
