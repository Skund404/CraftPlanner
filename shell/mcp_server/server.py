...
def create_server(api_client: httpx.AsyncClient | None = None) -> _LoggingFastMCP:
    """Create and configure the CraftPlanner MCP server.
    ...
    if api_client is None:
        ...
        shell_token = os.getenv("CRAFTPLANNER_SHELL_TOKEN", "")
        headers: dict = {}
        if shell_token:
    ...
    # e.g. CRAFTPLANNER_MCP_ALLOWED_HOSTS=craftplanner.duckdns.org
    _extra_hosts = [
        h.strip()
        for h in os.getenv("CRAFTPLANNER_MCP_ALLOWED_HOSTS", "").split(",")
        if h.strip()
    ]
    ...
    mcp = _LoggingFastMCP(
        name="craftplanner",
        instructions=(
            "You are connected to a CraftPlanner Shell — a modular project management toolkit "
