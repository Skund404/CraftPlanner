"""CraftPlanner Shell — FastAPI application entry point.
...
def _load_config() -> dict:
    ...
    return {
        "core_url": os.getenv("CRAFTPLANNER_CORE_URL", "http://localhost:8420"),
        "core_api_key": os.getenv("CRAFTPLANNER_CORE_API_KEY", ""),
        "userdb_path": os.getenv("CRAFTPLANNER_USERDB_PATH", "~/.craftplanner/userdb.sqlite"),
        "dev_mode": os.getenv("CRAFTPLANNER_DEV_MODE", "false").lower() in ("true", "1", "yes"),
        "port": int(os.getenv("CRAFTPLANNER_PORT", "3000")),
    }
...
def _configure_logging(dev_mode: bool) -> None:
    ...
    In production, logs are also written to ~/.craftplanner/logs/shell.log
    ...
    if dev_mode:
        ...
        log_dir = Path.home() / ".craftplanner" / "logs"
...
async def lifespan(app: FastAPI):
    ...
    print(
        f"\nCraftPlanner Shell v{SHELL_VERSION}\n"
...
def create_app() -> FastAPI:
    ...
    from .routers import backups, catalogue, data, dev, inventory, mcp_log, modules, settings, system, terminal, users, version, workshops

    app = FastAPI(
        title="CraftPlanner Shell",
        version=SHELL_VERSION,
        description=(
            "The CraftPlanner Shell API — proxy to the catalogue engine, "
