...
Install flow:
  ...
  2. Read 'targets' from craftplanner-package.json — a map of source → dest paths.
  3. Copy each file to its target (relative to craftplanner_home).
  4. Register in installed_packages.
  5. No restart required.

craftplanner-package.json for a data package:
{
  "name": "cyberpunk-theme",
  "type": "data",
  "version": "1.0.0",
  "targets": {
    "cyberpunk.json": ".craftplanner/themes/cyberpunk.json"
  }
}
...
class DataInstaller:
    ...
    def __init__(self, db: UserDB, craftplanner_home: Path) -> None:
        self._db = db
        self._home = craftplanner_home
    ...
    async def install(
        ...
        # Re-read craftplanner-package.json to get the 'targets' field (not in manifest model).
        pkg_json_path = package_dir / "craftplanner-package.json"
