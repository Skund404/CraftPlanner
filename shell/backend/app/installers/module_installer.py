...
class ModuleInstaller:
    ...
    async def install(
        ...
        if not module_manifest_path.exists():
            msg = (
                ...
                "A module package must contain both craftplanner-package.json and manifest.json."
            )
            await _fail_tx("validate_manifest", msg)
            return _fail_result(
                "validate_manifest", msg,
                "Ensure the package directory contains a manifest.json alongside craftplanner-package.json.",
            )
        ...
        if db_path != ":memory:":
            ...
            try:
                ...
                return _fail_result(
                    "snapshot_userdb", msg,
                    "Check available disk space in ~/.craftplanner/backups/ and try again.",
                )
    ...
    async def _run_migrations(self, module_name: str, package_path: str) -> None:
        ...
        for migration_file in sorted(migrations_dir.glob("[0-9]*.py")):
            module_key = (
                f"_craftplanner_module_{module_name.replace('-', '_')}_migration_{migration_file.stem}"
            )
            spec = importlib.util.spec_from_file_location(module_key, migration_file)
            if spec is None or spec.loader is None:
                raise RuntimeError(
    ...
    async def _run_migrations_down(self, module_name: str, package_path: str) -> None:
        ...
        for migration_file in sorted(migrations_dir.glob("[0-9]*.py"), reverse=True):
            module_key = (
                f"_craftplanner_rollback_{module_name.replace('-', '_')}_{migration_file.stem}"
            )
            spec = importlib.util.spec_from_file_location(module_key, migration_file)
            if spec is None or spec.loader is None:
                continue
