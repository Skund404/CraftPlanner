...
Backups live in ~/.craftplanner/backups/ alongside the database.
...
def _backups_dir(db: UserDB) -> Path:
    ...
    if db.path == ":memory:":
        return Path.home() / ".craftplanner" / "backups"
...
async def create_backup(db: UserDB = Depends(get_userdb)) -> BackupInfo:
    ...
    Creates a timestamped backup file in ~/.craftplanner/backups/ and returns
