...
# ---------------------------------------------------------------------------
# Workshop settings — primitive names + unit system
# ---------------------------------------------------------------------------

_DEFAULT_PRIMITIVE_NAMES: dict[str, str] = {
    "material": "Materials",
    "tool": "Tools",
...
class WorkshopSettings(_PydanticBaseModel):
    """Workshop-level settings: renamed primitive labels and unit system."""
    primitive_names: dict[str, str] = {}
    unit_system: str = "metric"  # "metric" or "imperial"


class WorkshopSettingsUpdate(_PydanticBaseModel):
    """Partial update for workshop settings."""
    primitive_names: dict[str, str] | None = None
    unit_system: str | None = None


@router.get(
    "/{workshop_id}/settings",
    response_model=WorkshopSettings,
    summary="Get workshop settings (primitive names, unit system)",
)
async def get_workshop_settings(
    workshop_id: str = FPath(...),
    db: UserDB = Depends(get_userdb),
) -> WorkshopSettings:
    """Return the workshop's custom settings, with defaults merged in."""
    await _get_or_404(workshop_id, db)

    import json as _json
    row = await db.fetch_one(
        "SELECT key, value FROM user_preferences WHERE user_id = ?",
        [f"workshop:{workshop_id}:settings"],
    )
    if row:
        try:
    ...
    names = {**_DEFAULT_PRIMITIVE_NAMES, **stored.get("primitive_names", {})}
    unit_system = stored.get("unit_system", "metric")
    return WorkshopSettings(primitive_names=names, unit_system=unit_system)
...
async def update_workshop_settings(
    ...
    """Partially update workshop settings (primitive names and/or unit system)."""
    await _get_or_404(workshop_id, db)

    import json as _json
    pref_key = f"workshop:{workshop_id}:settings"
    ...
    # Merge updates.
    if payload.primitive_names is not None:
        existing_names = stored.get("primitive_names", {})
        existing_names.update(payload.primitive_names)
        stored["primitive_names"] = existing_names

    if payload.unit_system is not None:
        if payload.unit_system not in ("metric", "imperial"):
            raise HTTPException(
                status_code=400,
                detail={"error": "unit_system must be 'metric' or 'imperial'"},
            )
        stored["unit_system"] = payload.unit_system
    ...
    await db.execute(
        ...
        ON CONFLICT (user_id, key) DO UPDATE SET value = excluded.value
        """,
        [pref_key, pref_key, _json.dumps(stored)],
    )

    log.info("workshop_settings_updated", workshop_id=workshop_id)
