...
def _load_package_manifest(package_path: str) -> PackageManifest:
    """Read and validate craftplanner-package.json from a package directory."""
    pkg_json = Path(package_path) / "craftplanner-package.json"
    if not pkg_json.exists():
        raise HTTPException(
            status_code=422,
            detail={
                "error": "craftplanner-package.json not found",
                "path": package_path,
                "suggestion": "The package directory must contain a craftplanner-package.json file.",
            },
        )
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail={
                "error": "craftplanner-package.json is not valid JSON",
                "detail": str(exc),
            },
        )
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail={
                "error": "craftplanner-package.json failed validation",
                "errors": exc.errors(),
            },
        )
...
async def update_package(
    ...
    Fetches the latest from the Git remote, re-reads craftplanner-package.json,
