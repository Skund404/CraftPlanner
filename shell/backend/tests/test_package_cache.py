...
def bare_repo(tmp_path: Path) -> Path:
    """Create a bare git repo with a tagged commit and a craftplanner-package.json."""
    ...
    # Add a craftplanner-package.json
    (source / "craftplanner-package.json").write_text(
...
async def test_fetch_clones_repo(cache: PackageCache, bare_repo: Path):
    ...
    assert path.exists()
    assert (path / "craftplanner-package.json").exists()
