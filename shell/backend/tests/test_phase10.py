...
def _pkg_manifest(fixture_name: str) -> PackageManifest:
    """Parse the craftplanner-package.json from a fixture directory."""
    import json
    pkg_json = FIXTURES / fixture_name / "craftplanner-package.json"
...
async def test_compat_fail_fails_at_step2(db):
    ...
    # compat-fail craftplanner-package.json has shell_compatibility >=99.0.0
    ...
    import json
    pkg_raw = json.loads((FIXTURES / "compat-fail" / "craftplanner-package.json").read_text())
...
async def test_skill_dry_run(db):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (pkg_path / "craftplanner-package.json").write_text(
            '{"name": "my-skill", "type": "skill", "version": "1.0.0"}'
        )
        (pkg_path / "skill.md").write_text("# My Skill\nSome content.")
        import json as _json
        raw = _json.loads((pkg_path / "craftplanner-package.json").read_text())
