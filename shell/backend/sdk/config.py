...
can override these by placing a JSON file at .craftplanner/modules/{name}.config.json
...
Usage in a module's routes.py:
    ...
    from craftplanner_sdk.config import ModuleConfig, get_module_config_factory
...
def get_module_config_factory(module_name: str, defaults: dict[str, Any] | None = None):
    ...
    async def _get_config(request: Request) -> ModuleConfig:
        ...
        # Phase 7+: fetch overrides from Core's .craftplanner/modules/{name}.config.json
